 ...
 */
public class TreeLayoutAlgorithm2 implements LayoutAlgorithm {
	 ...
	 */
	public TreeLayoutAlgorithm2(int direction) {
		this(direction, null);
	}
	 ...
	 */
	public TreeLayoutAlgorithm2(int direction, Dimension nodeSpace) {
	...
	void internalApplyLayout() {
		...
		for (Iterator iterator = superRoot.getChildren().iterator(); iterator
			...
			leafCountSoFar = leafCountSoFar + rootInfo.getNumOfLeaves();//.numOfLeaves;
		}
	}
	...
	private void updateLeafAndLayerSizes() {
		if (nodeSpace != null) {
			...
			TreeNode superRoot = treeObserver.getSuperRoot();
			if (direction == TOP_DOWN || direction == BOTTOM_UP) {
				leafSize = bounds.width / superRoot.getNumOfLeaves();
				layerSize = bounds.height / superRoot.getHeight();
			} else {
				leafSize = bounds.height / superRoot.getNumOfLeaves();
				layerSize = bounds.width / superRoot.getHeight();
			}
		}
	}
	...
	private void computePositionRecursively(TreeNode entityInfo,
		...
		double breadthPosition = relativePosition + entityInfo.getNumOfLeaves()
				/ 2.0;
		double depthPosition = (entityInfo.getDepth() + 0.5);
		...
		for (Iterator iterator = entityInfo.getChildren().iterator(); iterator
				.hasNext();) {
			TreeNode childInfo = (TreeNode) iterator.next();
			computePositionRecursively(childInfo, relativePosition);
			relativePosition += childInfo.getNumOfLeaves();
		}
	}
}
