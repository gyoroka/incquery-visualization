 ...
 */
public class SpringLayoutAlgorithm2 implements LayoutAlgorithm {
	 ...
	 */
	protected static final double MIN_DISTANCE = 30.0d;
	 ...
	 */
	public SpringLayoutAlgorithm2(int style) {
		this();
		setResizing(style != LayoutStyles.NO_LAYOUT_NODE_RESIZING);
	}

	public SpringLayoutAlgorithm2() {
	}
	...
	private void initLayout() {
		...
		for (int i = 0; i < connections.length; i++) {
			...
			//double weight = connection.getSource().getIncomingConnections().;
			...
			srcDestToSumOfWeights[source.intValue()][target.intValue()] += weight/(double)(connection.getTarget().getIncomingConnections().length+connection.getTarget().getOutgoingConnections().length+1);
			srcDestToSumOfWeights[target.intValue()][source.intValue()] += weight/(double)(connection.getSource().getIncomingConnections().length+connection.getSource().getOutgoingConnections().length+1);
		}
